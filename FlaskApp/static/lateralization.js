...
var testArea = function (){
    ...
    if (item.length > 0){
        startPage();
        getJsonApi({'type': 'trial', 'id': getID()}, apiUrl);
    }
}
var infoArea = function (){
    var item = $("body>section#info.active");
    if (item.length > 0){
        getJsonApi({'type': 'setup', 'id': getID()}, apiUrl);
    }
}
...
var lateralizationPlay = function (show_slider_value=null,nextContent=null,audio_file=null){
    // const audio = new Audio($("body").attr("calibration-audio"));
    const audio = new Audio(audio_file);
    audio.play();
    audio.addEventListener("ended",function (){

        $("body>section#testArea > section").html(nextContent);
        var handle = $( "#custom-handle" );

        if (show_slider_value){
            handle.addClass("with-text");
            $( "#slider" ).slider({
                value: 100,
                min: 0,
                max: 200,
                ...
                slide: function( event, ui ) {
                    if (ui.value > 100){
                        handle.text( "R " + (ui.value - 100) );
                    }else if(ui.value < 100){
                        var x = 100;
                        handle.text("L " + (x - ui.value))
                    }else{
                        handle.text("0");
                    }
                }
    ...
    serverResponse.done(function( data ) {
        switch (data.type) {
            ...
            case "level_calibration":
                ...
                var content = "<section>";
                if(data.lower_left_text){
                    content += '<span class="bottom-left">' + data.lower_left_text + '</span>';
                }
                if (data.lower_right_text){
                    content += '<span class="bottom-right">' + data.lower_right_text + '</span>';
                }
                if (data.upper_left_text){
                    content += '<span class="top-left">' + data.upper_left_text + '</span>';
                }

                content += '<p class="w-100 text-center">' + data.prompt + '</p><div class="d-flex">';

                if (data.loop_button){
                    content += "<div><button onclick='calibrationLoop(this)'>Loop</button></div>";
                }
            ...
            case "lateralization_trial":
                ...
                if (data.trial_pause){
                    bb.attr("trial_pause",data.trial_pause);
                }
                ...
                content += "<section>";
                content += '<p class="w-100 text-center">' + data.answer_prompt + '</p>' +
                    '<div style="width: 50%;margin: 80px auto 30px auto;">' +
                    '<div id="slider"><div id="custom-handle" class="ui-slider-handle"></div></div>' +
                    '<img alt="" src="static/img/head-white.svg"> </div>' +
                    '<button onclick="sendLateralizationAnswer()" style="margin-left: auto; margin-right: auto; display: block;">' + data.button_text + '</button>';
                content += "</section>";
                ...
                lateralizationPlay(data.show_slider_value,content,data.audio);
                break;

                // testArea.html(content);
                // var handle = $( "#custom-handle" );
                //
                // if (data.show_slider_value){
                //     handle.addClass("with-text");
                ...
                //
                // $("body").attr("calibration-audio",data.audio);
                // lateralizationPlay(data.show_slider_value,content);
                // calibrationPlay();
                break;
            default:
                break;
...
var sendLateralizationAnswer = function (){
    var answer = $("#slider").slider("value");
    setTimeout(function (){
        getJsonApi({'type': 'answer', 'value': answer}, apiUrl);
    },$("body").attr("trial_pause_dur"));
}
// var sendAnswerToApi = function (){
//     var answer = $("#howManyCorrect").val();
//     setTimeout(function (){
//         getJsonApi({'type': 'answer', 'value': answer},"lateralization_trial.json");
